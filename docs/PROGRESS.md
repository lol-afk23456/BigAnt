...
Il backlog **precedente** alla console (`2184776^`) contiene già B09: onboarding locali, piano/stato, configurazione servizi, consumi, identità amministrativa distinta, permessi e audit. Confrontati tale perimetro, AGENTS, SPEC, MISSIONS, implementazione e test della console: riusa Tenant/StaffUser e le impostazioni esistenti, separa PlatformAdmin, espone aggregati degli ospiti e non implementa impersonificazione o pagamenti automatici. PDF, credenziali e dati demo restano esclusi da Git.

Il run remoto `36036273444` del commit pubblicato era invece rosso: gate Lighthouse del menu a 95/100, LCP 2339 ms contro il limite invariato di 2000 ms. La riproduzione strumentata `36260553955` sul branch `codex/fix-menu-ci` conferma 9 scenari browser verdi e un solo scenario fallito sul benchmark finale (95/100, 2341 ms); tutte le verifiche funzionali del menu precedenti al benchmark passano. CSS, immagini e script arrivano entro circa 550 ms; la traccia mostra il primo Paint a 1321 ms e la presentazione a 2341 ms. Il confronto con display virtuale non risolve; il run `36261743082` misura invece 220 ms su HTML minimo, mentre il menu resta a circa 2340 ms anche con altre modalità di avvio. Non attribuiamo quindi il risultato a un rallentamento generale del runner. Gli esperimenti sono stati rimossi.

**Correzione:** il documento QR incorpora gli 8399 byte dei soli stili pubblici (`menu-styles.ts`), senza aspettare il CSS del pannello. Regole dei quattro temi trasferite, non ridisegnate; foglio amministrativo alleggerito. Conservati preload/varianti delle immagini, contenuto SSR, IT/EN, aggiornamento live e accessibilità. Test HTML aggiornati per impedire la reintroduzione della dipendenza CSS esterna; nessuna soglia Lighthouse ridotta. La CI conserva report JSON e tracce come artifact `browser-diagnostics` per sette giorni, anche quando passa.

**Verifiche sul Mac:** typecheck/lint/build verdi, 122 test backend verdi prima della modifica CSS e due test HTML aggiornati nuovamente verdi; 10/10 scenari browser finali in 2,6 minuti. Lighthouse 100/100 e LCP 626 ms sul caso sintetico della suite; 100/100 e 994 ms sulla copertina reale della demo. Sempre 750/250 Kbps, 150 ms e CPU 4×. Confronto delle proprietà e geometrie di tutti gli elementi nei quattro temi, a 375 e 1280 px: zero differenze; immagine Pub mobile prima/dopo identica al pixel e ispezionata. Impronte delle 26 tabelle demo identiche dopo i test: 244 prenotazioni, 107 ospiti, 42 tavoli e 42 piatti conservati. Demo web/API/worker riavviati; nessuna nuova migrazione. Report e confronti locali in `.local/ci-repair-20260926/`, esclusi da Git.

**Cancello Linux completato:** [run 36262708002](https://github.com/lol-afk23456/BigAnt/actions/runs/36262708002) sul commit applicativo `8837e5b`: typecheck, lint, tutti i 122 test backend, build e 10/10 scenari browser verdi, nessun test saltato o retry. Lighthouse **100/100, LCP 429 ms**, FCP 312 ms; eliminata la richiesta a `bigant.css`. Il push finale su `main` è stato esplicitamente richiesto dall'utente dopo test e risoluzione; integrazione del risultato finale senza gli script diagnostici temporanei. Nessun deploy o invio reale; PDF della console ancora locale.

## Guida PDF della console — 24 settembre 2026

Su richiesta dell'utente, generato `output/pdf/BigAnt_Book_Console_Amministratore_Guida_2026-09.pdf`, manuale locale di nove pagine riferito al commit `2184776`. Spiega scopo e ruoli, accesso e navigazione, onboarding, configurazione servizi, esempio con tavoli da 2/4/4, gestione commerciale/operatori/stati, dodici prove manuali con risultati attesi e scheda di segnalazione. Nomi dei comandi e comportamenti confrontati con guida, codice e test; immagine della Panoramica dalla demo originale, senza credenziali segrete. Le prove descritte sono istruzioni per il collaboratore, non risultati di una nuova sessione di collaudo.

PDF renderizzato e ispezionato in tutte le pagine; corretta la spaziatura del diagramma, ricontrollati numerazione, testi, margini e quattro collegamenti. File esplicitamente escluso da Git, come gli altri PDF richiesti dall'utente; nessun push, invio o cambiamento applicativo in questo blocco. Dati e servizi della demo conservati. Il cancello applicativo resta quello M5A riportato sotto.
