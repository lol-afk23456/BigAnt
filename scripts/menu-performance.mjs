...
const chromeFlags=process.env.MENU_AUDIT_HEADFUL==='1'?'--no-sandbox':'--headless --no-sandbox';
execFileSync(process.execPath,['node_modules/lighthouse/cli/index.js',url,'--quiet','--only-categories=performance','--output=json',`--output-path=${output}`,'--save-assets',`--chrome-flags=${chromeFlags}`,'--throttling-method=devtools','--throttling.requestLatencyMs=150','--throttling.downloadThroughputKbps=750','--throttling.uploadThroughputKbps=250','--throttling.cpuSlowdownMultiplier=4'],{stdio:'pipe',timeout:150000});
