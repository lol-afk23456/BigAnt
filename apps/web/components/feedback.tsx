...
export function PublicFeedback({slug}:{slug:string}){
 ...
 const privateHeading=useRef<HTMLHeadingElement>(null),resultHeading=useRef<HTMLHeadingElement>(null);
 useEffect(()=>{if(privateForm)privateHeading.current?.focus();},[privateForm]);
 useEffect(()=>{if(receipt)resultHeading.current?.focus();},[receipt]);
 ...
 async function send(channel:'private'|'google_redirect',event?:FormEvent){
  event?.preventDefault();if(busy||receipt)return;const data=publicReviewInput.safeParse({channel,...(options?.card_uid?{card_uid:options.card_uid}:{}),...(channel==='private'?{rating,comment}:{})});
 ...
 return <div className="public-shell"><header className="public-header"><Brand href={`/r/${slug}`}/><LanguageSwitch/></header><main className="feedback-shell">
  {unavailable?<section className="panel feedback-error"><p>{t('feedbackUnavailable')}</p><a className="button secondary" href={`/r/${slug}`}>{t('backVenue')}</a></section>:!options?error?<ErrorNotice message={error} onRetry={()=>setRevision(r=>r+1)}/>:<Loading/>:<>
   ...
   <div className={`feedback-options${options.google_available?'':' feedback-single'}`} aria-busy={busy}>
    ...
    <button className="panel feedback-option" type="button" disabled={busy||!!receipt} aria-expanded={privateForm} aria-controls="private-feedback" onClick={()=>{setPrivateForm(true);setError('');}}><span aria-hidden="true">✉</span><strong>{t('privateOption')}</strong></button>
   </div>
   {options.google_demo&&<p className="small muted feedback-demo">{t('googleDemoNotice')}</p>}
   {error&&<ErrorNotice message={error}/>}
   {receipt?<section className="panel feedback-result" role="status"><span className="success-icon" aria-hidden="true">✓</span><h2 ref={resultHeading} tabIndex={-1}>{t(receipt.channel==='private'?'feedbackThanks':receipt.google_demo?'googleDemoTitle':'googleOpeningTitle')}</h2><p>{t(receipt.channel==='private'?'feedbackReceived':receipt.google_demo?'googleDemoHint':'googleOpeningHint')}</p>{receipt.channel==='google_redirect'&&!receipt.google_demo&&receipt.redirect_url&&<a className="button secondary" href={receipt.redirect_url}>{t('googleOption')} ↗</a>}<a className="button secondary" href={`/r/${slug}`}>{t('backVenue')}</a></section>:privateForm&&<form id="private-feedback" className="panel feedback-form reveal" aria-busy={busy} onSubmit={event=>void send('private',event)}><h2 ref={privateHeading} tabIndex={-1}>{t('privateTitle')}</h2><p className="muted">{t('privateHint')}</p><fieldset disabled={busy}><legend>{t('feedbackRating')}</legend><div className="feedback-rating">{[1,2,3,4,5].map(value=><label key={value} className={rating===value?'selected':''}><input type="radio" name="rating" value={value} required checked={rating===value} onChange={()=>setRating(value)} aria-label={`${t('ratingChoice')} ${value}`}/><span>{value}</span></label>)}</div></fieldset><label>{t('feedbackComment')}<textarea disabled={busy} value={comment} onChange={event=>setComment(event.target.value)} maxLength={2000} rows={4}/></label><p className="small muted">{t('feedbackPrivacyHint')}</p><button className="button primary wide" type="submit" disabled={busy}>{t(busy?'saving':'sendFeedback')} →</button></form>}
