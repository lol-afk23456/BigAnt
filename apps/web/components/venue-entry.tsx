  ...
  <main className="demo-home venue-home">
   {!venue?error?<ErrorNotice message={error} onRetry={retry}/>:<Loading/>:<>
    ...
    <div className="venue-grid">
     <article className="panel venue-card"><span className="venue-symbol" aria-hidden="true">✳</span><h2>{t('book')}</h2><p className="muted">{t('bookingCardHint')}</p><p className="small muted">{t(venue.auto_confirm?'autoConfirmHint':'manualConfirmHint')}</p><a className="button primary" href={`/r/${slug}/prenota`}>{t('book')} →</a></article>
    ...
    <div className="venue-home-footer"><a className="text-button" href={`/r/${slug}/feedback`}>{t('shareExperience')}</a>{venue.phone&&<a className="text-button" href={`tel:${venue.phone}`}>{t('callVenue')}</a>}<a className="text-button" href={`/r/${slug}/staff`}>{t('staffLink')} ↗</a></div>
