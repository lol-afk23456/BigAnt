...
export function Brand({compact=false}:{compact?:boolean}){const {t}=useCopy();return <a href="/" className="brand" aria-label={t('brand')}><span className="brand-mark" aria-hidden="true">b<span>•</span></span>{!compact&&<span>{t('brand')}</span>}</a>;}
