...
(() => {
  ...
  async function check() {
    ...
    try {
      ...
      if (!next || next.dataset.menuUrl !== current.dataset.menuUrl || next.dataset.version === current.dataset.version || !current.isConnected) return;
      ...
      if (focused) Array.from(next.querySelectorAll('a')).find(node => node.getAttribute('href') === focused)?.focus({preventScroll:true});
